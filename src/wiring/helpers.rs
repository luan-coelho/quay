...
//! `format_runtime`, `format_relative_time`). The only function that
//! touches state is [`auto_tag_seed_tasks`], which seeds Phase-4 colour
//! labels onto demo tasks at startup.
...
pub fn auto_tag_seed_tasks(state: &AppState) -> anyhow::Result<()> {
